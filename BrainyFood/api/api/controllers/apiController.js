...
const ProduitCheckout = require('../models/prodcheckout.model');
...
const stripe = require('stripe')('sk_test_51Gu5l8AL9qNlrkR4Whoxnc4FPDsgi1bGkqy2HHpt7pyLqUZSkTXobV0P4HW7FFMMUaXKR8blN39B2wXKZ27MbXVz00pP48nKJg')
...
const wooapi = new WooCommerceRestApi({
    url: "https://smrt28c3050551e74506985fbca107b0bf05.1.rocketbonds.ch/",
    consumerKey: "ck_867796667bbbbb5a61877914844c2a5d66aed62f",
    consumerSecret: "cs_9127463582ceceabadf55abe38b90d1e2567403c",
    version: "wc/v3"
});

exports.getAllProductsLabelVin = function (req, res) {
...
exports.getAllOrders = function (req, res) {
    ...
    wooapi.get("orders",{order:"desc"}).then((response) => {
        ret.status = response.status;
        ret.data = response.data;
        res.status(ret.status);
        res.json(ret);
...
exports.createProspect = function (req,res){




    const new_prospect = new Prospect(req.body)

    if(req.body.constructor === Object && Object.keys(req.body).length === 0){
        res.status(400).send({ error:true, message: 'Please provide all required field' });
    }else{
        Prospect.create(new_prospect, function(err, employee) {
            if (err)
                res.send(err);
            res.json({error:false,message:"Prospect added successfully!",data:employee});
        });
    }
...
exports.getProspects=function(req,res){
    Prospect.findAll( function(err, employee) {
        if (err)
            res.send(err);
        res.json({"status":200,"data":employee,error:false});
    });
...
exports.sendNewsLetterMail = function (req, res) {
    let transporter = nodemailer.createTransport({
        host: 'smtp.gmail.com',
        port: 587,
        secure: false, // true for 465, false for other ports
    ...
    ejs.renderFile(path.join(__dirname, '../controllers/newsletters/'+nom),{}, function (err, data) {
        if (err) {
            console.log(err);
        } else {
            var mainOptions = {
...
exports.uploadImageProduct=function uploadImageProduct (req,res) {
  console.log("work")
    const tempPath = req.file.path;
    if (path.extname(req.file.originalname).toLowerCase() === ".png") {
        const targetPath = path.join(__dirname, "./product_images/"+req.file.filename+".png");
    ...
    } else if (path.extname(req.file.originalname).toLowerCase() === ".jpg" ||path.extname(req.file.originalname).toLowerCase() === ".jpeg" ) {
        const targetPath = path.join(__dirname, "./product_images/"+req.file.filename+".jpeg");
...
exports.uploadImageNewsletter=function uploadImageNewsletter (req,res) {
    console.log("work")
    const tempPath = req.file.path;
    if (path.extname(req.file.originalname).toLowerCase() === ".png") {
        const targetPath = path.join(__dirname, "./newsletters_images/"+req.file.filename+".png");
        ...
        const targetPath = path.join(__dirname, "./newsletters_images/"+req.file.filename+".jpeg");
...
exports.uploadHtmlFile=function uploadHtmlFile (req,res) {
    console.log("work")
    const tempPath = req.file.path;
    if (path.extname(req.file.originalname).toLowerCase() === ".html") {
        const targetPath = path.join(__dirname, "./newsletters/"+req.file.filename+".html");
        ...
        fs.unlink(tempPath, err => {
            ...
            res
                .status(403)
                .contentType("text/plain")
                .end("Only .html files are allowed!");
        });
    }
...
exports.getProductImage=function getProductImage(req,res) {
    var filename=req.params.filename
    res.sendFile(path.join(__dirname, "./product_images/"+filename));

}
exports.getNewsletterImage=function getProductImage(req,res) {
    var filename=req.params.filename
    res.sendFile(path.join(__dirname, "./newsletters_images/"+filename));

}

exports.createProduit = function (req,res){
...
exports.createNewsletter = function (req,res){




    const new_newsletter = new Newsletter(req.body)
    console.log(req.body)

    if(req.body.constructor === Object && Object.keys(req.body).length === 0){
        res.status(400).send({ error:true, message: 'Please provide all required field' });
    }else{
        Newsletter.create(new_newsletter, function(err, newsletter) {
            if (err)
                res.send(err);
            res.json({error:false,message:"Newsletter added successfully!",data:newsletter});
        });
    }
...
exports.getNewsletters=function getNewsletters(req,res){
    Newsletter.findAll( function(err, employee) {
        if (err)
            res.send(err);
        res.json(employee);
    });
...
exports.getProduits=function getProduits(req,res){
    Produit.findAll( function(err, employee) {
        if (err)
            res.send(err);
        res.json(employee);
    });
...
exports.getProduitById = function getProduitById(req, res) {
    Produit.findById(req.params.id, function(err, employee) {
        if (err)
            res.send(err);
        res.json(employee);
    });
...
exports.createCheckout = function (req,res){
    ...
    const new_checkout = new Checkout(req.body)
    console.log(req.body)

    if(req.body.constructor === Object && Object.keys(req.body).length === 0){
        res.status(400).send({ error:true, message: 'Please provide all required field' });
    }else{
        Checkout.create(new_checkout, function(err, checkout) {
            if (err)
                res.send(err);
            res.json({error:false,message:"checkout added successfully!",data:checkout});
        });
    }
...
exports.createProduitCheckout = function (req,res){




    const new_prodCheckout = new ProduitCheckout(req.body)
    console.log(req.body)

    if(req.body.constructor === Object && Object.keys(req.body).length === 0){
        res.status(400).send({ error:true, message: 'Please provide all required field' });
    }else{
        ProduitCheckout.create(new_prodCheckout, function(err, checkout) {
            if (err)
                res.send(err);
            res.json({error:false,message:"checkout added successfully!",data:checkout});
        });
...
exports.getCheckout= function (req,res) {
    ...
    ProduitCheckout.findByIdCheckout(id,function (err,produits) {
        if (!err){
            Checkout.findById(id,function (err,checkout) {
                if (!err){

                    console.log(checkout[0].id_user)

                    Patient.findByEmail(checkout[0].email_user,function (err,patient) {
                        res.status(200)
                        res.json({
                            produits:produits,
                            client:patient
